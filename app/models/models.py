...
from sqlalchemy import func, and_, or_, Column, select, Integer, String, Date, CHAR, update
...
class BaseModel(Base):
    ...
    def to_dict(self, results):
        return [{c.name: getattr(result, c.name) for c in self.__table__.columns} for result in results]
    ...
    def get_all_db(cls):
        with session_scope() as session:
            ...
            return [{c.name: getattr(result, c.name) for c in cls.__table__.columns} for result in results]
    ...
    def get_result_view_all(cls, page, *args, **kwargs):
        ...
        if page.session.get('/home').ch_contractor.value:
            query = query.where(
                and_(Decedent.situation != '手続終了', Decedent.situation != 'キャンセル', Decedent.situation != ''))
        ...
        if 'dict' in kwargs:
            ...
            for key, value in kwargs['dict'].items():
                ...
                if key == '依頼人：姓':
                    query = query.where(cls.username1.like(f'%{value}%'))
                if key == '電話番号':
                    query = query.filter(or_(cls.contact_home.like(f'%{value}%'), cls.contact_phone.like(f'%{value}%')))
