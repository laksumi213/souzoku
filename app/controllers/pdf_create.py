...
class PdfCreate:
    ...
    def init_set(self, obj):
        ...
        pdfmetrics.registerFont(TTFont('IPAexGothic', os.path.dirname(__file__) + '/ipaexg.ttf'))
        ...
        with open(os.path.dirname(__file__) + '_tmp.pdf', 'rb') as fp:
            ...
            for page in PDFPage.create_pages(PDFDocument(PDFParser(fp))):
                ...
                for element in layout:
                    ...
                    if hasattr(element, "get_text") and element.get_text().splitlines()[-1] == str(t):
                        pos = [round(element.bbox[0] * 0.352778),
                               round(element.bbox[1] * 0.3538),
                               element.bbox[2] * 0.352778,
                               element.bbox[3] * 0.35
    ...
    def pdf_save(self, output_name, marge_pdf=None, page=1, open_bool=False):
        ...
        with open(output_name, 'wb') as f:
            ...
            if open_bool:
                if os.name == 'nt':
                    subprocess.Popen(
                        ['C:\Program Files\Adobe\Acrobat DC\Acrobat\Acrobat.exe', output_name.replace('/', '\\')])
                elif os.name == 'posix':
    ...
    @classmethod
    def a3_marge(cls, save_path='./', *args, ):
        ...
        for i, arg in enumerate(args):
            merger.append(
                os.path.join(
                    ...
                    os.path.splitext(args[i])[0] + '_output.pdf'
                )
            )
        ...
        output_file = os.path.join(save_path, os.path.basename(
            str(os.path.splitext(args[0])[0]).replace('1', '') + "_A3.pdf"))  # 見開きにしたPDFの保存
...
if __name__ == '__main__':
    ...
    pdf.pdf_save(os.path.join(r'\\192.168.11.20\行政書士法人チェスター\08.その他\スキャン\森町', '111'), os.path.join(os.path.dirname(os.getcwd()), 'pdf', 'SBI証券_個人情報に関する開示等請求書.pdf'), page=2, open_bool=True)
