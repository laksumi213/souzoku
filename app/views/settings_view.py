...
class SettingsBody(BaseView):
    ...
    def clicked_name_registration(self, e):
        def registration(_):
            ...
            with open(
                utils.get_config_path(), "w", encoding="CP932", errors="ignore"
            ) as f:
                config.write(f)
        ...
        try:
            ...
            tf_mail.value = config.get(os.getlogin(), "mail").replace(
                "@tax-info.jp", ""
            )
        ...
        super().page.dialog = AlertDialog(
            ...
            actions=[eb_registration, eb_cancel],
...
class StaffRegistration(BaseView):
    def __init__(self):
        ...
        if len(self.staff) == 0:
            row_items = [
                TextField(label="氏", width=150, hint_text="name1"),
                TextField(label="名", data="name2", width=150),
                TextField(label="ふりがな(姓)", data="name1_huri", width=150),
                TextField(label="ふりがな(名)", data="name2_huri", width=150),
                TextField(label="メール", data="mail"),
                TextField(label="電話番号", data="tel", width=150),
            ]
            ...
            for item in self.staff:
                row_items.append(
                    [
                        Text(value=item["staff_id"]),
                        TextField(label="氏", value=item["name1"]),
